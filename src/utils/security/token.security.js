...
// helper function to normalize expiresIn
const normalizeExpiresIn = (value) => {
  if (!value) return "15m"; // default 15 minutes
  // لو قيمة رقمية
  if (!isNaN(value)) return Number(value);
  // لو مكتوبة كـ string زي "15m" أو "7d"
  return value;
};
...
export const getLoginCredentials = async ({ user } = {}) => {
  ...
  const access_token = await generateToken({
    ...
    options: {
      expiresIn: normalizeExpiresIn(process.env.ACCESS_TOKEN_EXPIRES_IN),
      jwtid: jwtId,
    },
  });
  ...
  const refresh_token = await generateToken({
    ...
    options: {
      expiresIn: normalizeExpiresIn(process.env.REFRESH_TOKEN_EXPIRES_IN),
      jwtid: jwtId,
    },
  });
    ...
    data: [
      {
        ...
        expiresIn:
          req.decoded.iat +
          normalizeExpiresIn(process.env.REFRESH_TOKEN_EXPIRES_IN),
